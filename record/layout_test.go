...
func TestNewLayout(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expectedSize:  64, // utils.IntSize(header) + 8(date) + utils.IntSize(int) + (utils.IntSize + 10*4)(varchar)
			expectedAlign: map[string]int{
				"timestamp": 8,
				"count":     utils.IntSize,
				"name":      1,
		...
		},
		{
			...
			expectedOrder: []string{"created", "id", "count", "type", "active", "name"},
			expectedSize:  96, // utils.IntSize(header) + 8(date) + 8(long) + utils.IntSize(int) + 2(short) + 1(bool) + (utils.IntSize + 15*4)(varchar)
			expectedAlign: map[string]int{
				"created": 8,
				"id":      8,
				"count":   utils.IntSize,
...
func TestLayoutPaddingOptimization(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expectedSize: 40, // utils.IntSize(header) + 8(long) + utils.IntSize(int) + (utils.IntSize + 3*4)(varchar) + 1(bool) + padding
		},
	}
